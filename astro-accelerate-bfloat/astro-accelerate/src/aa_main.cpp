...
float g_target_dm = 0.0;
std::string g_output_dat_filepath;

int main(int argc, char *argv[]) {

	//float g_target_dm; //defined in aa_params.hpp
	g_target_dm = std::stof(argv[2]);
	for (int i = 2; i < argc - 1; i++){
		argv[i] = argv[i+1];
	}
	argc = argc - 1;

	g_output_dat_filepath = argv[2];
	for (int i = 2; i < argc - 1; i++){
		argv[i] = argv[i+1];
	}
	argc = argc - 1;
