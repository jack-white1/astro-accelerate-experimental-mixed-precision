...
namespace astroaccelerate {
  ...
  __global__ void cuda_convolve_reg_1d_halftemps(__nv_bfloat162* d_kernel, __nv_bfloat162* d_signal, __nv_bfloat162* d_ffdot_plane,int sig_tot_convlen, float scale)
  {
    ...
    __nv_bfloat162 tempkern[nTemplates]; // half template array, 1 column per thread
...
#pragma unroll
	...
	d_ffdot_plane[(NKERN - 1 - i)*sig_tot_convlen + j + tidx ].x = (local_data.x*tempkern[i].x + local_data.y*tempkern[i].y) * (__nv_bfloat16)scale;
	d_ffdot_plane[(NKERN - 1 - i)*sig_tot_convlen + j + tidx ].y = (-local_data.x*tempkern[i].y + local_data.y*tempkern[i].x) * (__nv_bfloat16)scale;
      }

      //do z=0
      __nv_bfloat162 tempz = __ldg(&d_kernel[nTemplates*KERNLEN + tidx]) ;
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].x = (local_data.x*tempz.x - local_data.y*tempz.y) * (__nv_bfloat16)scale;
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].y = (local_data.x*tempz.y + local_data.y*tempz.x) * (__nv_bfloat16)scale;
    }
  }
