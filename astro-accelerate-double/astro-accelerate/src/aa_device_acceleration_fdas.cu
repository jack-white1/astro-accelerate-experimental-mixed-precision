...
namespace astroaccelerate {
  ...
  printf("\nCMDARGS.NORM: %d\n", cmdargs.norm);
    ...
    */
	...
	//memory required
	size_t mem_ffdot = params.ffdotlen * sizeof(double); // total size of ffdot plane powers in bytes
	printf("!!!!!!!!!MEM_FFDOT double = %zu",mem_ffdot);
	size_t mem_ffdot_cpx = params.ffdotlen_cpx * sizeof(double2); // total size of ffdot plane powers in bytes
	size_t mem_kern_array = KERNLEN * NKERN * sizeof(double2);
	size_t mem_max_list_size = params.max_list_length*4*sizeof(double);
	size_t mem_signals = (params.nsamps * sizeof(double)) + (params.siglen * sizeof(double2)) + (params.extlen * sizeof(double2));
	...
	//printf("SIZE OF double %zu, SIZE OF FLOAT %zu\n",sizeof(double), sizeof(float));

	//Allocating gpu arrays
	gpuarrays.mem_insig = params.nsamps * sizeof(double);
	gpuarrays.mem_rfft = params.rfftlen * sizeof(double2);
	gpuarrays.mem_extsig = params.extlen * sizeof(double2);
	...
	// Calculate kernel templates on CPU and upload-fft on GPU
	 ...
	 fdas_create_double_acc_kernels(gpuarrays.d_kernel, gpuarrays.temp_kernel, &cmdargs);
	 printf(" done.\n");
	//getLastCudaError("\nCuda Error\n");
	...
	// FFT
	 for (int i = 0; i < range; i++) {
	    ...
	    /*double *double_output_buffer = (double*)malloc(processed*sizeof(double));
	    for (int j = 0; j<processed; j++){
	    	double_output_buffer[j] = (double)output_buffer[i][dm_count][j];
	    }*/
...
/*#ifndef NOCUST
	    ...
	    if(cmdargs.basic || cmdargs.kfft){
	      ...
	      fdas_transfer_gpu_arrays_double_to_float(&gpuarrays_float,&gpuarrays,&cmdargs);
	      ...
	      /*
	      double2 *test_double;
	      test_double = (double2*) malloc(4);
	      (*test_double).x = (double)1024;
	      (*test_double).y = (double)0;

	      printf("test_double: %p, *test_double: %hu\n", test_double, *(unsigned int*)test_double);
			*/
...
*/
	      ...
	      fdas_write_ffdot(gpuarrays.d_ffdot_pwr, &cmdargs, &params, dm_low[i], dm_count, dm_step[i]);
	      exit(1);
#endif
	  ...
	  if (enable_output_ffdot_plan && (dm_count == best_dm))
	  {
		fdas_write_ffdot((double*)(gpuarrays.d_ffdot_pwr), &cmdargs, &params, dm_low[i], dm_count, dm_step[i]);
	  }
