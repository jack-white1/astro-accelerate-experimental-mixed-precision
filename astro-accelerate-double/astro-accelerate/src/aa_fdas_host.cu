...
namespace astroaccelerate {
  ...
  cudaError_t fdas_alloc_gpu_arrays(fdas_gpuarrays *arrays,  cmd_args *cmdargs)
  {
    ...
    //templates
    e = cudaMalloc((void**)&arrays->d_kernel, KERNLEN*sizeof(double2)*NKERN );
    ...
    printf("ffdot: %zu, size: %zu, NKERN: %zu\n",(unsigned long)arrays->mem_ffdot,(unsigned long)sizeof(double),(unsigned long)NKERN);
    printf("ffdot x size: %lu",(unsigned long)arrays->mem_ffdot/sizeof(double)/(unsigned long)NKERN);
    if(cmdargs->basic==1){
  ...
  LOG(log_level::error, "Could not cudaMalloc in aa_fdas_host.cu (" + std::string(cudaGetErrorString(e)) + ")");
    ...
    //call_kernel_cast_double2_to_float2(out_arrays->d_fft_signal, in_arrays->d_fft_signal, in_arrays->mem_rfft);
    //call_kernel_cast_double2_to_float2(out_arrays->d_ext_data, in_arrays->d_ext_data, in_arrays->mem_extsig);
    //call_kernel_cast_double2_to_float2(out_arrays->d_kernel, in_arrays->d_kernel, KERNLEN*sizeof(double2)*NKERN);
    //call_kernel_cast_double2_to_float2(out_arrays->d_ffdot_cpx, in_arrays->d_ffdot_cpx, in_arrays->mem_ffdot_cpx);

  void fdas_transfer_gpu_arrays_double_to_float(fdas_gpuarrays_float *out_arrays, fdas_gpuarrays *in_arrays, cmd_args *cmdargs)
  {
    printf("\nCopying + converting gpu arrays (only d_ffdot_pwr) from double into float, \n");

    call_kernel_cast_double_to_float(out_arrays->d_ffdot_pwr, in_arrays->d_ffdot_pwr, in_arrays->mem_ffdot);

    //printf("GPU array copy/conversion to single precision from double completed\n");
  }

void compare_host_1D_arrays(float* h_float_array, double* h_double_array, size_t data_length_bytes){
  double true_arr_length = data_length_bytes/sizeof(float);
  int arr_length = (int) true_arr_length;
  printf("\n true_arr_length: %lf\n", true_arr_length);
  for (int i = 0; i<arr_length; i++){
    printf("d_in_signal[%d] =    double: %f, float: %f, difference: %f\n",i,(float)h_double_array[(int)i],h_float_array[(int)i],(float)h_double_array[(int)i]-(float)h_float_array[(int)i]);
  }
}

void compare_1D_arrays(float* d_float_array, double* d_double_array, size_t data_length_bytes){
  double *h_double_array;
  float *h_float_array;
  
  h_double_array =  (double*)  malloc(data_length_bytes*2);
  h_float_array =     (float*)          malloc(data_length_bytes);

  cudaMemcpy((void*)h_double_array, (void*)d_double_array,  data_length_bytes*2,    cudaMemcpyDeviceToHost);
  cudaMemcpy((void*)h_float_array,    (void*)d_float_array,     data_length_bytes,  cudaMemcpyDeviceToHost);

  compare_host_1D_arrays(h_float_array, h_double_array, data_length_bytes);
}

void compare_host_1D2_arrays(float2* h_float2_array, double2* h_double2_array, size_t data_length_bytes){
  double true_arr_length = data_length_bytes/sizeof(float2);
  int arr_length = (int) true_arr_length;
  printf("\n true_arr_length: %lf\n", true_arr_length);
  for (int i = 0; i<arr_length; i++){
    printf("arr[%d].x =    double: %lf, float: %f\n",i,h_double2_array[(int)i].x,h_float2_array[(int)i].x);
    printf("arr[%d].y =    double: %lf, float: %f\n",i,h_double2_array[(int)i].y,h_float2_array[(int)i].y);
  }
}

void compare_1D2_arrays(float2* d_float2_array, double2* d_double2_array, size_t data_length_bytes){
  double2 *h_double2_array;
  float2 *h_float2_array;
  
  h_double2_array =  (double2*)  malloc(data_length_bytes*2);
  h_float2_array =     (float2*)          malloc(data_length_bytes);

  cudaMemcpy((void*)h_double2_array, (void*)d_double2_array,  data_length_bytes*2,    cudaMemcpyDeviceToHost);
  cudaMemcpy((void*)h_float2_array,    (void*)d_float2_array,     data_length_bytes,  cudaMemcpyDeviceToHost);

  compare_host_1D2_arrays(h_float2_array, h_double2_array, data_length_bytes);
}

void print_1D_double_array(double* d_double_array, size_t data_length_bytes){
  double *h_double_array;

  h_double_array =  (double*)  malloc(data_length_bytes);

  cudaMemcpy((void*)h_double_array, (void*)d_double_array,data_length_bytes,cudaMemcpyDeviceToHost);

  for (int i = 0; i < data_length_bytes/sizeof(double); i++){
    /*if (h_double_array[i]!=h_double_array[i]){
      printf("%f\n",h_double_array[i]);
    }*/
    printf("%f\n",h_double_array[i]);
...
void print_1D_double2_array(double2* d_double2_array, size_t data_length_bytes){
  double2 *h_double2_array;

  h_double2_array =  (double2*)  malloc(data_length_bytes);

  cudaMemcpy((void*)h_double2_array, (void*)d_double2_array,data_length_bytes,cudaMemcpyDeviceToHost);

  for (int i = 0; i < data_length_bytes/sizeof(double2); i++){

    printf("x: %lf, y: %lf\n",h_double2_array[i].x, h_double2_array[i].y);
  }
}
  ...
  void fdas_create_double_acc_kernels(double2* d_kernel, float2* temp_kernel_pointer, cmd_args *cmdargs) {
    ...
    call_kernel_cast_float2_to_double2(d_kernel, temp_kernel_pointer,KERNLEN*sizeof(float2)* NKERN);
    //compare_1D2_arrays(temp_kernel_pointer, d_kernel, 2048);
  }
...
*/
  ...
  void fdas_cuda_create_fftplans(fdas_cufftplan *fftplans, fdas_params *params) {
    ...
    cufftResult e = cufftEstimate1d( params->nsamps, CUFFT_D2Z, 1, &real_worksize);
    ...
    e = cufftEstimateMany(nrank, (int*) n, (int*) inembed, istride, idist, (int*) onembed, ostride, odist, CUFFT_Z2Z, params->nblocks, &sig_worksize);
    ...
    if(CUFFT_SUCCESS != cufftCreate(&fftplans->realplan)) {
    ...
    if(CUFFT_SUCCESS != cufftCreate(&fftplans->forwardplan)) {
      LOG(log_level::error, "Could not cufftCreate in aa_fdas_host.cu ");
    }


    e = cufftMakePlanMany(fftplans->forwardplan, nrank, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2Z, params->nblocks, &sig_worksize);

    if(e != CUFFT_SUCCESS) {
      LOG(log_level::error, "Could not cufftMakePlanMany in aa_fdas_host.cu 2");
    }
  ...
  void fdas_cuda_basic(fdas_cufftplan *fftplans, fdas_gpuarrays *gpuarrays, cmd_args *cmdargs, fdas_params *params)
  {
    ...
    */
    ...
    double2 *f2temp;
    double *ftemp;
    ftemp  = (double *)malloc(params->rfftlen*sizeof(double));
    f2temp = (double2 *)malloc(params->rfftlen*sizeof(double2));
    cudaError_t e = cudaMemcpy(ftemp, gpuarrays->d_in_signal, (params->rfftlen)*sizeof(double), cudaMemcpyDeviceToHost);
    //printf("\nPRINTING D_IN_SIGNAL\n");
    //print_1D_double_array(gpuarrays->d_in_signal, (params->rfftlen)*sizeof(double));
    ...
    e = cudaMemcpy(gpuarrays->d_fft_signal, f2temp, (params->rfftlen)*sizeof(double2), cudaMemcpyHostToDevice);
    ...
    if (cmdargs->norm){
      ...
      fftsig = (double2*)malloc((params->rfftlen)*sizeof(double2));
      fftsig_float2 = (float2*)malloc((params->rfftlen)*sizeof(float2));

      cudaError_t e = cudaMemcpy(fftsig, gpuarrays->d_fft_signal, (params->rfftlen)*sizeof(double2), cudaMemcpyDeviceToHost);
      ...
      for (int i = 0;i < params->rfftlen; i++){
        fftsig[i].x = (double) fftsig_float2[i].x;
        fftsig[i].y = (double) fftsig_float2[i].y;
      }

      e = cudaMemcpy(gpuarrays->d_fft_signal, fftsig, (params->rfftlen)*sizeof(double2), cudaMemcpyHostToDevice);
    ...
    if (cmdargs->norm){
      ...
      extsig = (double2*)malloc((params->extlen)*sizeof(double2));
      cudaError_t e = cudaMemcpy(extsig, gpuarrays->d_ext_data, (params->extlen)*sizeof(double2), cudaMemcpyDeviceToHost);
	...
	presto_norm(extsig_float2+b*KERNLEN, KERNLEN);
      ...
      for (int i = 0;i < params->extlen; i++){
        extsig[i].x = (double) extsig_float2[i].x;
        extsig[i].y = (double) extsig_float2[i].y;
      }


      e = cudaMemcpy(gpuarrays->d_ext_data, extsig, (params->extlen)*sizeof(double2), cudaMemcpyHostToDevice);
    ...
    //complex block fft
    cufftExecZ2Z(fftplans->forwardplan,  gpuarrays->d_ext_data,  gpuarrays->d_ext_data, CUFFT_FORWARD);
    //printf("AFTER CUFFT===========================================================================\n\n\n\n\n\n\n\n\n\n");
    //print_1D_double2_array(gpuarrays->d_ext_data, gpuarrays->mem_extsig);
    ...
    for (int k=0; k < nTemplates; k++){
      cufftExecZ2Z(fftplans->forwardplan, (gpuarrays->d_ffdot_cpx + k * params->extlen), (gpuarrays->d_ffdot_cpx + k *params->extlen), CUFFT_INVERSE);
      cufftExecZ2Z(fftplans->forwardplan, (gpuarrays->d_ffdot_cpx + (NKERN-1-k) * params->extlen), (gpuarrays->d_ffdot_cpx + (NKERN-1-k) *params->extlen), CUFFT_INVERSE);
    }
    // z=0
    cufftExecZ2Z(fftplans->forwardplan, gpuarrays->d_ffdot_cpx + (nTemplates * params->extlen), gpuarrays->d_ffdot_cpx + (nTemplates * params->extlen), CUFFT_INVERSE);
...
*/
  ...
  void fdas_write_list(fdas_gpuarrays_float *gpuarrays, cmd_args *cmdargs, fdas_params *params, float *h_MSD, float dm_low, int dm_count, float dm_step, unsigned int list_size){
    ...
    //LOG(log_level::error, "FDAS_WRITE_LIST_CALLED");
	...
	mkdir(dirname, 0700);
      ...
      FILE *fp_c;
      char pfname[400];
      sprintf(pfname, "%speaks.dat",g_output_dat_filepath.c_str());
  ...
  void fdas_write_ffdot(double* d_ffdot_pwr , cmd_args *cmdargs, fdas_params *params, float dm_low, int dm_count, float dm_step ) {
    ...
    double *h_ffdotpwr = (double*)malloc(params->ffdotlen* sizeof(double));
    //download data
    cudaError_t e = cudaMemcpy(h_ffdotpwr, d_ffdot_pwr, params->ffdotlen*sizeof(double), cudaMemcpyDeviceToHost);
    ...
    sprintf(pfname, "%splane.dat",g_output_dat_filepath.c_str());
    ...
    //fprintf(fp_c, "%.2f\t%.3f\t%u\t%.3f\t%.3f\t%.3f\n", temp_acc, temp_acc1, temp_j , temp_jfreq, temp_pow, temp_sigma);
