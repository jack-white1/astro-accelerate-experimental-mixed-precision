...
namespace astroaccelerate {
  ...
  static __device__ __inline__ void do_FFT_no_reorder(double2 *s_input, int N, int bits){ // in-place
    double2 A_DFT_value, B_DFT_value, ftemp2, ftemp;
    double2 WB;
  ...
  static __device__ __inline__ void do_FFT_CT_DIF_2elem_no_reorder(double2 *s_input){
    double2 A_DFT_value, B_DFT_value;
    double2 W;
    double2 Aftemp, Bftemp;
    ...
    for(q=4;q>=0;q--){
      ...
      Aftemp.x = (double)parity*A_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value.x, PoTm1);
      Aftemp.y = (double)parity*A_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value.y, PoTm1);
      Bftemp.x = (double)parity*B_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.x, PoTm1);
      Bftemp.y = (double)parity*B_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.y, PoTm1);
  ...
  static __device__ __inline__ void do_FFT_CT_DIF_4elem_no_reorder(double2 *s_input){
    ...
    for(q=4;q>=0;q--){
      ...
      Bftemp.x = (double)parity*B_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.x, PoTm1);
      Bftemp.y = (double)parity*B_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.y, PoTm1);
      Cftemp.x = (double)parity*C_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value.x, PoTm1);
      Cftemp.y = (double)parity*C_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value.y, PoTm1);
      Dftemp.x = (double)parity*D_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value.x, PoTm1);
      Dftemp.y = (double)parity*D_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value.y, PoTm1);
  ...
  static __device__ __inline__ void do_IFFT_no_reorder(double2 *s_input, int N, int bits){ // in-place
    double2 A_DFT_value, B_DFT_value;
    double2 W;
    double2 ftemp, ftemp2;
    ...
    if(threadIdx.x<Nhalf){
      ...
      A_DFT_value.x=(double)parity*A_DFT_value.x+ aa_shfl(AA_ASSUME_MASK,A_DFT_value.x,local_id+parity);
      A_DFT_value.y=(double)parity*A_DFT_value.y+ aa_shfl(AA_ASSUME_MASK,A_DFT_value.y,local_id+parity);
		
      B_DFT_value.x=(double)parity*B_DFT_value.x+ aa_shfl(AA_ASSUME_MASK,B_DFT_value.x,local_id+parity);
      B_DFT_value.y=(double)parity*B_DFT_value.y+ aa_shfl(AA_ASSUME_MASK,B_DFT_value.y,local_id+parity);
  ...
  static __device__ __inline__ void do_IFFT_mk11_2elem_2vertical_no_reorder(double2 *s_input_1, double2 *s_input_2){
    double2 A_DFT_value1, B_DFT_value1;
    double2 A_DFT_value2, B_DFT_value2;
    double2 W;
    double2 Aftemp1, Bftemp1;
    double2 Aftemp2, Bftemp2;
    ...
    A_DFT_value1.x=(double)parity*A_DFT_value1.x + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value1.x,1);
    A_DFT_value1.y=(double)parity*A_DFT_value1.y + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value1.y,1);
    B_DFT_value1.x=(double)parity*B_DFT_value1.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value1.x,1);
    B_DFT_value1.y=(double)parity*B_DFT_value1.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value1.y,1);
	
    A_DFT_value2.x=(double)parity*A_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value2.x,1);
    A_DFT_value2.y=(double)parity*A_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value2.y,1);
    B_DFT_value2.x=(double)parity*B_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value2.x,1);
    B_DFT_value2.y=(double)parity*B_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value2.y,1);
    ...
    for(q=1;q<5;q++){
      ...
      A_DFT_value1.x = Aftemp1.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp1.x,PoT);
      A_DFT_value1.y = Aftemp1.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp1.y,PoT);
      B_DFT_value1.x = Bftemp1.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp1.x,PoT);
      B_DFT_value1.y = Bftemp1.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp1.y,PoT);
		
      A_DFT_value2.x = Aftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp2.x,PoT);
      A_DFT_value2.y = Aftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp2.y,PoT);
      B_DFT_value2.x = Bftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp2.x,PoT);
      B_DFT_value2.y = Bftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp2.y,PoT);	
  ...
  static __device__ __inline__ void do_IFFT_mk11_2elem_no_reorder(double2 *s_input){
    double2 A_DFT_value, B_DFT_value;
    double2 W;
    double2 ftemp, ftemp2;
    ...
    A_DFT_value.x=(double)parity*A_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value.x,1);
    A_DFT_value.y=(double)parity*A_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value.y,1);
	
    B_DFT_value.x=(double)parity*B_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.x,1);
    B_DFT_value.y=(double)parity*B_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.y,1);
    ...
    for(q=1;q<5;q++){
      ...
      A_DFT_value.x = ftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,ftemp2.x,PoT);
      A_DFT_value.y = ftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,ftemp2.y,PoT);
      B_DFT_value.x = ftemp.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,ftemp.x,PoT);
      B_DFT_value.y = ftemp.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,ftemp.y,PoT);
  ...
  static __device__ __inline__ void do_IFFT_mk11_4elem_no_reorder(double2 *s_input){
    double2 A_DFT_value, B_DFT_value, C_DFT_value, D_DFT_value;
    double2 W;
    double2 Aftemp, Bftemp, Cftemp, Dftemp;
    ...
    B_DFT_value.x=(double)parity*B_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.x,1);
    B_DFT_value.y=(double)parity*B_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value.y,1);
    C_DFT_value.x=(double)parity*C_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value.x,1);
    C_DFT_value.y=(double)parity*C_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value.y,1);
    D_DFT_value.x=(double)parity*D_DFT_value.x + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value.x,1);
    D_DFT_value.y=(double)parity*D_DFT_value.y + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value.y,1);
    ...
    for(q=1;q<5;q++){
      ...
      A_DFT_value.x = Aftemp.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp.x,PoT);
      A_DFT_value.y = Aftemp.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp.y,PoT);
      B_DFT_value.x = Bftemp.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp.x,PoT);
      B_DFT_value.y = Bftemp.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp.y,PoT);
      C_DFT_value.x = Cftemp.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp.x,PoT);
      C_DFT_value.y = Cftemp.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp.y,PoT);
      D_DFT_value.x = Dftemp.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp.x,PoT);
      D_DFT_value.y = Dftemp.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp.y,PoT);	
  ...
  static __device__ __inline__ void do_IFFT_mk11_4elem_2vertical_no_reorder(double2 *s_input1, double2 *s_input2){
    double2 A_DFT_value1, B_DFT_value1, C_DFT_value1, D_DFT_value1;
    double2 A_DFT_value2, B_DFT_value2, C_DFT_value2, D_DFT_value2;
    double2 W;
    double2 Aftemp1, Bftemp1, Cftemp1, Dftemp1;
    double2 Aftemp2, Bftemp2, Cftemp2, Dftemp2;
    ...
    B_DFT_value1.x=(double)parity*B_DFT_value1.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value1.x,1);
    B_DFT_value1.y=(double)parity*B_DFT_value1.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value1.y,1);
    C_DFT_value1.x=(double)parity*C_DFT_value1.x + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value1.x,1);
    C_DFT_value1.y=(double)parity*C_DFT_value1.y + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value1.y,1);
    D_DFT_value1.x=(double)parity*D_DFT_value1.x + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value1.x,1);
    D_DFT_value1.y=(double)parity*D_DFT_value1.y + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value1.y,1);
	
    A_DFT_value2.x=(double)parity*A_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value2.x,1);
    A_DFT_value2.y=(double)parity*A_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,A_DFT_value2.y,1);
    B_DFT_value2.x=(double)parity*B_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value2.x,1);
    B_DFT_value2.y=(double)parity*B_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,B_DFT_value2.y,1);
    C_DFT_value2.x=(double)parity*C_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value2.x,1);
    C_DFT_value2.y=(double)parity*C_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,C_DFT_value2.y,1);
    D_DFT_value2.x=(double)parity*D_DFT_value2.x + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value2.x,1);
    D_DFT_value2.y=(double)parity*D_DFT_value2.y + aa_shfl_xor(AA_ASSUME_MASK,D_DFT_value2.y,1);
    ...
    for(q=1;q<5;q++){
      ...
      B_DFT_value1.x = Bftemp1.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp1.x,PoT);
      B_DFT_value1.y = Bftemp1.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp1.y,PoT);
      C_DFT_value1.x = Cftemp1.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp1.x,PoT);
      C_DFT_value1.y = Cftemp1.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp1.y,PoT);
      D_DFT_value1.x = Dftemp1.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp1.x,PoT);
      D_DFT_value1.y = Dftemp1.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp1.y,PoT);
		
      A_DFT_value2.x = Aftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp2.x,PoT);
      A_DFT_value2.y = Aftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Aftemp2.y,PoT);
      B_DFT_value2.x = Bftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp2.x,PoT);
      B_DFT_value2.y = Bftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Bftemp2.y,PoT);
      C_DFT_value2.x = Cftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp2.x,PoT);
      C_DFT_value2.y = Cftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Cftemp2.y,PoT);
      D_DFT_value2.x = Dftemp2.x + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp2.x,PoT);
      D_DFT_value2.y = Dftemp2.y + (double)parity*aa_shfl_xor(AA_ASSUME_MASK,Dftemp2.y,PoT);	
  ...
  static __device__ inline double pwcalc(double2 cpx)
  {
    double pw = (cpx.x*cpx.x + cpx.y*cpx.y);
    return pw;
  ...
  __global__ void cuda_overlap_copy(double2* d_ext_data, double2* d_cpx_signal, int sigblock, int sig_rfftlen, int sig_tot_convlen, int kern_offset, int total_blocks)
  {
    ...
    for (int i = tid; i < sig_tot_convlen; i += numthreads){
      d_ext_data[i].x = (double) 0.0f;
      d_ext_data[i].y = (double) 0.0f;
    }
  ...
  __global__ void cuda_convolve_reg_1d_halftemps(double2* d_kernel, double2* d_signal, double2* d_ffdot_plane,int sig_tot_convlen, float scale)
  {
    ...
    const int nTemplates = ((NKERN-1)/2);

    double2 tempkern[nTemplates]; // half template array, 1 column per thread
    for (int i=0; i < nTemplates; i++ )
      tempkern[i] = __ldg(&d_kernel[i*KERNLEN + tidx]) ;

    for (int j = 0; j < sig_tot_convlen; j+=KERNLEN){
      double2 local_data = __ldg(&d_signal[tidx + j]);
#pragma unroll
      for (int i = 0; i < nTemplates; i++){
	//upper half
	d_ffdot_plane[i*sig_tot_convlen + j + tidx ].x = (local_data.x*tempkern[i].x - local_data.y*tempkern[i].y) * (double)scale;
	d_ffdot_plane[i*sig_tot_convlen + j + tidx ].y = (local_data.x*tempkern[i].y + local_data.y*tempkern[i].x) * (double)scale;
	//complex conjugate filter lower half
	d_ffdot_plane[(NKERN - 1 - i)*sig_tot_convlen + j + tidx ].x = (local_data.x*tempkern[i].x + local_data.y*tempkern[i].y) * (double)scale;
	d_ffdot_plane[(NKERN - 1 - i)*sig_tot_convlen + j + tidx ].y = (-local_data.x*tempkern[i].y + local_data.y*tempkern[i].x) * (double)scale;
      }

      //do z=0
      double2 tempz = __ldg(&d_kernel[nTemplates*KERNLEN + tidx]) ;
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].x = (local_data.x*tempz.x - local_data.y*tempz.y) * (double)scale;
      d_ffdot_plane[nTemplates*sig_tot_convlen + j + tidx ].y = (local_data.x*tempz.y + local_data.y*tempz.x) * (double)scale;
    }
  }
  ...
  /* calculates complex signal power and places result in float f-fdot array */
  {
    ...
    for (int i = 0; i < total_blocks; ++i){
      if (tidx < sigblock){
        double2  local_data =  __ldg(&d_ffdot_plane_cpx[(tidy * sig_tot_convlen) + i*KERNLEN  + tidx + kern_offset ]) ;
  ...
  /* This one performs interbinning on the complex signal before power calculations */
  {
    ...
    __shared__ double2 local_data[PTBSIZEX + 1];
    __shared__ double local_data_inbin[2*PTBSIZEX];
	...
	local_data_inbin[2*tx + 1] =  (double)0.616850275f * ((local_data[tx].x - local_data[tx+1].x) * (local_data[tx].x - local_data[tx+1].x) + (local_data[tx].y - local_data[tx+1].y) * (local_data[tx].y - local_data[tx+1].y));
	__syncthreads();
...
void call_kernel_cast_float_to_double(double *d_output, float *d_input, size_t data_length_bytes){
  ...
  cast_double_to_float<<<N_threads/threads_per_block, threads_per_block>>>(d_output, d_input, data_length_per_thread);
}

__global__ void cast_float_to_double(double *d_output, float *d_input, size_t data_length_bytes){
  unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
  for (unsigned long long i = 0; i < (unsigned long long) data_length_bytes/sizeof(float); i++){
    d_output[idx + i] = (double) (d_input[idx+i]);
  }
}
...
__global__ void cast_double_to_float(float *d_output, double *d_input, size_t data_length_bytes){
  unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
  for (unsigned long long i = 0; i < (unsigned long long) data_length_bytes/sizeof(double); i++){
...
void call_kernel_cast_double2_to_float2(float2 *d_output, double2 *d_input, size_t data_length_bytes){
  ...
  unsigned long long threads_per_block = 1;
  size_t data_length_per_thread = (size_t)(((unsigned long long)data_length_bytes)/N_threads);
  cast_double2_to_float2<<<N_threads/threads_per_block, threads_per_block>>>(d_output, d_input, data_length_per_thread);
}

__global__ void cast_double2_to_float2(float2 *d_output, double2 *d_input, size_t data_length_bytes){
  unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
  for (unsigned long long i = 0; i < (unsigned long long) data_length_bytes/sizeof(double2); i++){
...
void call_kernel_cast_float_to_double(double *d_output, float *d_input, size_t data_length_bytes){
  ...
  //printf("Casting float to double: N_floats: %llu, N_blocks: %llu, true_N_floats: %lf, true_N_blocks: %lf\n", N_floats, N_blocks, true_N_floats, true_N_blocks);
  cast_float_to_double<<<N_blocks, N_threads>>>(d_output, d_input, N_floats);
}

__global__ void cast_float_to_double(double *d_output, float *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx] = (double) d_input[idx];
    }
}
...
void call_kernel_cast_float2_to_double2(double2 *d_output, float2 *d_input, size_t data_length_bytes){
  ...
  cast_float2_to_double2<<<N_blocks, N_threads>>>(d_output, d_input, N_float2s);
}

__global__ void cast_float2_to_double2(double2 *d_output, float2 *d_input, unsigned long long N_floats){
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N_floats){
      d_output[idx].x = (double) d_input[idx].x;
      d_output[idx].y = (double) d_input[idx].y;
    }
}
...
void call_kernel_cast_double_to_float(float *d_output, double *d_input, size_t data_length_bytes){
  int N_threads = 32;
  unsigned long long N_floats = data_length_bytes/sizeof(double);
  unsigned long long N_blocks = N_floats/N_threads;
  //double true_N_floats = ((double)data_length_bytes)/((double)sizeof(double));
  //double true_N_blocks = true_N_floats/((double)N_threads);

  //printf("Casting double to float: N_floats: %llu, N_blocks: %llu, true_N_floats: %lf, true_N_blocks: %lf\n", N_floats, N_blocks, true_N_floats, true_N_blocks);
  cast_double_to_float<<<N_blocks, N_threads>>>(d_output, d_input, N_floats);
}
...
void call_kernel_cast_double2_to_float2(float2 *d_output, double2 *d_input, size_t data_length_bytes){
  int N_threads = 32;
  unsigned long long N_float2s = data_length_bytes/sizeof(double2);
  unsigned long long N_blocks = N_float2s/N_threads;
  //double true_N_floats = ((double)data_length_bytes)/((double)sizeof(double2));
  //double true_N_blocks = true_N_floats/((double)N_threads);

  //printf("Casting double2 to float2: N_float2s: %llu, N_blocks: %llu, true_N_float2s: %lf, true_N_blocks: %lf\n", N_float2s, N_blocks, true_N_floats, true_N_blocks);
  cast_double2_to_float2<<<N_blocks, N_threads>>>(d_output, d_input, N_float2s);
}
...
/*
  ...
  __global__ void customfft_fwd_temps_no_reorder(double2* d_signal)
  {
    ...
    __shared__ double2 s_input[KERNLEN]; 
  ...
  // In this version we apply Karel's kernel with removed de-shuffling of the data during the FFT.
  {
    ...
    int bx = blockIdx.x;
    double2 ffdotcpx;
    double2 ffdotcpx_trans;
    double2 local_data; //fft'd data
    ...
    double2 A_DFT_value, B_DFT_value, sA_DFT_value, sB_DFT_value;
    double2 ftemp2, ftemp, stemp2, stemp;
    double2 W;
    int local_id, warp_id;
    int j, m_param;
    //int load_id, i, n;
    ...
    __shared__ double2 s_input[KERNLEN]; //signal input data to FFT
    __shared__ double2 s_input_trans[KERNLEN]; //static allocation
...
#pragma unroll 
	...
	sA_DFT_value.x=(double)parity*sA_DFT_value.x+ aa_shfl(AA_ASSUME_MASK,sA_DFT_value.x,local_id+parity);
	sA_DFT_value.y=(double)parity*sA_DFT_value.y+ aa_shfl(AA_ASSUME_MASK,sA_DFT_value.y,local_id+parity);
	sB_DFT_value.x=(double)parity*sB_DFT_value.x+ aa_shfl(AA_ASSUME_MASK,sB_DFT_value.x,local_id+parity);
	sB_DFT_value.y=(double)parity*sB_DFT_value.y+ aa_shfl(AA_ASSUME_MASK,sB_DFT_value.y,local_id+parity);
  ...
  __global__ void cuda_convolve_customfft_wes_no_reorder02_inbin(double2* d_kernel, double2* d_signal, double *d_ffdot_pw, int sigblock, int sig_tot_convlen, int sig_totlen, int offset, float scale, double2 *ip_edge_points)
  // convolution kernel using Karel Adamek's custom FFT, deployed here with modifications by Wes Armour.
  // It performs the forward FFT and then loops through filters.(1-d blocks)
  // It also uses half the templates and computes the rest using the complex conjugate.
  // The modifications are optimizing speed by computing FFTs on two templates with one synchronization point. 
...
#pragma unroll 
	...
	// interpolated points
	s_output_ip[2*tx+1] = (double)0.616850275f * ((s_input[tx+offset].x - s_input[tx+1 + offset].x) * (s_input[tx +offset].x - s_input[tx+1 +offset].x) + (s_input[tx+offset].y - s_input[tx+1+offset].y) * (s_input[tx+offset].y - s_input[tx+offset+1].y)); 
	s_output_ip_trans[2*tx+1] = (double)0.616850275 * ((s_input_trans[tx+offset].x - s_input_trans[tx+1+offset].x) * (s_input_trans[tx+offset].x - s_input_trans[tx+1+offset].x) + (s_input_trans[tx+offset].y - s_input_trans[tx+1+offset].y) * (s_input_trans[tx+offset].y - s_input_trans[tx+1+offset].y)); 
      }
      __syncthreads();
  ...
  __global__ void GPU_CONV_kFFT_mk11_2elem_2v(double2 const* __restrict__ d_input_signal, double *d_output_plane_reduced, double2 const* __restrict__ d_templates, int useful_part_size, int offset, int nConvolutions, double scale) {
    __shared__ double2 s_input_1[KERNLEN];
    __shared__ double2 s_input_2[KERNLEN];
    // Convolution
    double2 r_templates_1[2];
    double2 r_templates_2[2];
    double2 signal[2];
    int pos, t;
    // Loading data
  ...
  __global__ void GPU_CONV_kFFT_mk11_4elem_2v(double2 const* __restrict__ d_input_signal, double *d_output_plane_reduced, double2 const* __restrict__ d_templates, int useful_part_size, int offset, int nConvolutions, double scale) {
    __shared__ double2 s_input_1[KERNLEN];
    __shared__ double2 s_input_2[KERNLEN];
    // Convolution
    double2 r_templates_1[4];
  ...
  void call_kernel_GPU_CONV_kFFT_mk11_4elem_2v(const dim3 &grid_size, const dim3 &block_size, double2 const*const d_input_signal, double *const d_output_plane_reduced, double2 const*const d_templates, const int &useful_part_size, const int &offset, const int &nConvolutions, const double &scale) {
