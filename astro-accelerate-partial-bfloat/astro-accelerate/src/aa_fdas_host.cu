...
void compare_host_1D2_arrays(float2* h_float2_array, __nv_bfloat162* h_bfloat162_array, size_t data_length_bytes){
  double true_arr_length = data_length_bytes/sizeof(__nv_bfloat162);
  int arr_length = (int) true_arr_length;
  printf("\n true_arr_length: %lf\n", true_arr_length);
  for (int i = 0; i<arr_length; i++){
    printf("array[%d] = bf16.x: %f, bf16.y: %f, fp32.x: %f, fp32.y: %f\n",i,(float)h_bfloat162_array[(int)i].x,(float)h_bfloat162_array[(int)i].y,h_float2_array[(int)i].x,h_float2_array[(int)i].y);
  }
}

void compare_1D2_arrays(float2* d_float2_array, __nv_bfloat162* d_bfloat162_array, size_t data_length_bytes){
  __nv_bfloat162 *h_bfloat162_array;
  float2 *h_float2_array;
  
  h_bfloat162_array =  (__nv_bfloat162*)  malloc(data_length_bytes);
  h_float2_array =     (float2*)          malloc(data_length_bytes*2);

  cudaMemcpy((void*)h_bfloat162_array, (void*)d_bfloat162_array,  data_length_bytes,    cudaMemcpyDeviceToHost);
  cudaMemcpy((void*)h_float2_array,    (void*)d_float2_array,     data_length_bytes*2,  cudaMemcpyDeviceToHost);

  compare_host_1D2_arrays(h_float2_array, h_bfloat162_array, data_length_bytes);
}
...
void print_1D_bfloat162_array(__nv_bfloat162* d_bfloat162_array, size_t data_length_bytes){
  ...
  void fdas_create_bfloat_acc_kernels(__nv_bfloat162* d_kernel, float2* temp_kernel_pointer, cmd_args *cmdargs) {
    ...
    //printf("Printing bfloat_kernels:\n");
    //compare_1D2_arrays(temp_kernel_pointer, d_kernel, KERNLEN*sizeof(__nv_bfloat162)*NKERN);
    //print_1D_bfloat162_array(d_kernel, KERNLEN*sizeof(__nv_bfloat162)* NKERN);
...
*/
  ...
  void fdas_cuda_create_fftplans(fdas_cufftplan *fftplans, fdas_params *params) {
    ...
    long long int ll_n[] = {KERNLEN};
    long long int ll_idist = n[0], ll_odist =n[0];
    long long int *ll_inembed = ll_n, *ll_onembed = ll_n;
    long long int ll_istride =1, ll_ostride = 1;
    ...
    long long int ll_rn[] = {params->nsamps};
    long long int *ll_rinembed = ll_rn, *ll_ronembed = ll_rn;
    long long int ll_ridist = ll_rn[0], ll_rodist = params->rfftlen;
    ...
    cufftXtGetSizeMany(fftplans->forwardplan, nrank, ll_rn, ll_inembed, istride, ll_idist,  CUDA_C_16BF,ll_onembed, ostride, ll_odist,
        CUDA_C_16BF, params->nblocks, &sworksize,CUDA_C_16BF);

    if(e != CUFFT_SUCCESS) {
      LOG(log_level::error, "Could not cufftCreate in aa_fdas_host.cu");
    }
    printf("\nWHILE MAKING PLANS, N: %d", KERNLEN);
        e = cufftXtMakePlanMany(fftplans->forwardplan, nrank, ll_n, ll_inembed, ll_istride, ll_idist, CUDA_C_16BF,
        ll_onembed, ll_ostride, ll_odist,CUDA_C_16BF, params->nblocks, &sworksize,CUDA_C_16BF);
    if(e != CUFFT_SUCCESS) {
      LOG(log_level::error, "Could not cufftMakePlanMany in aa_fdas_host.cu");
    }
  ...
  void fdas_cuda_basic(fdas_cufftplan *fftplans, fdas_gpuarrays *gpuarrays, cmd_args *cmdargs, fdas_params *params)
  {
    ...
    */
    ...
    for (int k=0; k < nTemplates; k++){
      ...
      cufftXtExec(fftplans->forwardplan, (gpuarrays->d_ffdot_cpx + (NKERN-1-k) * params->extlen), (gpuarrays->d_ffdot_cpx + (NKERN-1-k) *params->extlen), CUFFT_INVERSE);
    }
    // z=0
    cufftXtExec(fftplans->forwardplan, gpuarrays->d_ffdot_cpx + (nTemplates * params->extlen), gpuarrays->d_ffdot_cpx + (nTemplates * params->extlen), CUFFT_INVERSE);
...
*/
  ...
  public:
      ...
      void assign(float acc, float acc1, int j, float jfreq, float pow, float SNR) {
          this->acc   = acc;
          this->acc1  = acc1;
          this->j     = j;
          this->jfreq = jfreq;
      ...
      void print() {
          std::cout << acc << " " << acc1 << " " << j << " " << jfreq << " " << pow << " " << SNR << std::endl;
      }

      bool operator > (const fdasResult &A) const {
          return (this->SNR > A.SNR);
      }
  ...
  bool compareBySNR(const fdasResult &A, const fdasResult &B) {
      return A.SNR > B.SNR;
  }
  ...
  void fdas_write_ffdot(float* d_ffdot_pwr , cmd_args *cmdargs, fdas_params *params, float dm_low, int dm_count, float dm_step ) {
    ...
    //prepare file
    const char *dirname= "/home/jack/hdd/dat/";
    struct stat st = {0};
    ...
    sprintf(pfname, "%splane.dat",g_output_dat_filepath.c_str());//, dm_low + ((float)dm_count)*dm_step);
    ...
    printf("NKERN, ibin*params->siglen = %d, %d\n",NKERN,ibin*params->siglen);
    float temp_pow = 0;
    double temp_acc = 0;
    double temp_acc1 = 0;
    long long int temp_j = 0;
    double temp_jfreq = 0;
    float temp_sigma = 0;

    std::vector<fdasResult> peaks;
    fdasResult peak;

    for(long long int a = 0; a < NKERN; a++) {
      double acc = (double) (ZMAX - a* ACCEL_STEP);
      //printf("acc: %lf\n", acc);
      for(long long int j = 0; j < ibin*params->siglen; j++){
        ...
        if (pow > temp_pow){
          ...
          temp_jfreq = jfreq;
          temp_sigma = sigma;
          //printf("New peak at jfreq = %lf, accn = %lf, pow = %f\n", temp_jfreq, temp_acc, temp_pow);
        }
        //peak.assign(acc, acc1, j , jfreq, pow, sigma);
        //peaks.push_back(peak);

        fprintf(fp_c, "%.2lf\t%.3lf\t%lld\t%.3lf\t%.3f\t%.3f\n", acc, acc1, j , jfreq, pow, sigma);
        }
      //}
    }
    ...
    //printf("Sorting peaks..\n");

    //std::sort(peaks.begin(),peaks.end(),compareBySNR);

    //printf("Finished sorting peaks\n");
